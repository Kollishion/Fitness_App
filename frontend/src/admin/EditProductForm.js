...
const EditProductForm = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:8080/admins/products/get/${productId}`)
      .then(response => {
        if (!response.ok) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    fetch(`http://localhost:8080/admins/products/update/${productId}`, {
      method: 'POST',
      headers: {
      ...
      .then(response => {
        ...
        navigate('/admin/product-data'); // Redirect to the product data page
      })
      .catch(error => setError(error.message));
  };
